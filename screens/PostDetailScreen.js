...
const PostDetailScreen  = () => {
  ...
  try {
    // Lấy tất cả các bài viết của user hiện tại
    ...
    // Thêm thông tin người dùng vào mỗi bài viết
    const postsWithUserInfo = await Promise.all(
      postsData.map(async (post) => {
        const userRef = doc(db, "users", post.uid); // Tham chiếu đến user
        const userSnap = await getDoc(userRef);
    ...
    const sortedPosts = postsWithUserInfo.sort((a, b) => {
      ...
      return timeB - timeA; // Sắp xếp từ mới đến cũ
    });

    setPosts(sortedPosts); // Cập nhật state
  } catch (error) {
      ...
      <View style={styles.postContainer}>
        <View style={styles.header}>
          ...
          <TouchableOpacity  style={styles.optionsButton}>
            <MaterialCommunityIcons name="dots-horizontal" size={25} color="#1d3557" />
        </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  optionsButton: {
  position: "absolute",
  right: 10, // Căn sát vào bên phải, thay đổi giá trị này nếu cần
  top: "45%",
  transform: [{ translateY: -12.5 }], // Đặt dấu ba chấm căn giữa theo chiều dọc
},
});
